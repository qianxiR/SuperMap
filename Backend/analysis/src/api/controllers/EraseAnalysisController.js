 ...
 * 擦除分析控制器 - API层
 */
const { EraseAnalysisRequestDTO } = require('../../application/dtos/EraseAnalysisDTO');
const EraseAnalysisUseCase = require('../../application/useCases/EraseAnalysisUseCase');
const LayerRepository = require('../../infrastructure/repositories/LayerRepository');
const fs = require('fs').promises;
const path = require('path');
const { v4: uuidv4 } = require('uuid');

class EraseAnalysisController {
  /**
   * 擦除分析控制器构造函数
   * 
   * 输入数据格式：
   ...
   * 
   * 输出数据格式：
   * EraseAnalysisController实例
   */
  constructor(dependencies = {}) {
    this.layerRepository = dependencies.layerRepository || new LayerRepository();
    this.eraseAnalysisUseCase = new EraseAnalysisUseCase({
   ...
   * JSON格式的API响应
   */
  async executeEraseAnalysis(req, res) {
    try {
      console.log('[EraseAnalysisController] 收到擦除分析请求');

      // 验证请求数据
      this._validateRequest(req);

      // 创建请求DTO
      const requestDTO = new EraseAnalysisRequestDTO(req.body);

      console.log('[EraseAnalysisController] 请求DTO创建成功:', {
        targetFeatures: requestDTO.targetData.features?.length || 0,
        eraseFeatures: requestDTO.eraseData.features?.length || 0,
        targetData: {
          type: requestDTO.targetData.type,
          featuresCount: requestDTO.targetData.features?.length
        },
        eraseData: {
          type: requestDTO.eraseData.type,
          featuresCount: requestDTO.eraseData.features?.length
        }
      });

      // 执行分析用例
      const responseDTO = await this.eraseAnalysisUseCase.execute(requestDTO);

      console.log('[EraseAnalysisController] 分析完成:', {
      ...
      console.error('[EraseAnalysisController] 擦除分析执行失败:', error);
  ...
  _validateRequest(req) {
    ...
    const { targetData, eraseData } = req.body;
    ...
    if (eraseData.type !== 'FeatureCollection') {
      throw new Error('擦除图层数据必须是FeatureCollection格式');
    }
    ...
    if (!Array.isArray(eraseData.features)) {
      throw new Error('擦除图层要素数据必须是数组格式');
    }

    // 几何要素过滤现在在领域服务层统一处理
    console.log('[EraseAnalysisController] 数据验证完成，几何处理将在领域服务层执行');
  }
  ...
  _handleError(error, res) {
    console.error('[EraseAnalysisController] 处理错误:', error);
...
module.exports = EraseAnalysisController;
