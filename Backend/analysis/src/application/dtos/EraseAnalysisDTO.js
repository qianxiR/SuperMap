...
class EraseAnalysisRequestDTO {
   ...
   * @param {Object} data.eraseData - 擦除图层GeoJSON数据
   ...
   * 擦除数据对象
   */
  _createEraseData(eraseData) {
    if (!eraseData || !eraseData.type) {
      throw new Error('擦除图层数据格式无效');
    }

    return {
      type: eraseData.type,
      features: eraseData.features || [],
      properties: eraseData.properties || {}
    };
  }
  ...
  _createOptions(options) {
    ...
    return {
      resultLayerName: opts.resultLayerName || '擦除分析结果',
...
class EraseAnalysisResponseDTO {
  /**
   * 擦除分析响应DTO构造函数
   * 
   * 输入数据格式：
   * @param {Object} data - 响应数据
   * @param {Array} data.results - 擦除分析结果数组
   ...
   */
  constructor(data) {
    this.id = data.id || `erase_${Date.now()}`;
    this.name = data.name || '擦除分析结果';
  ...
  _formatResults(results) {
    return results.map((result, index) => ({
      id: result.id || `erase_${index}_${Date.now()}`,
      name: result.name || `擦除区域 ${index + 1}`,
      geometry: result.geometry,
      sourceTargetLayerName: result.sourceTargetLayerName || '目标图层',
      sourceEraseLayerName: result.sourceEraseLayerName || '擦除图层',
  ...
  _createStatistics(statistics) {
    return {
      ...
      eraseFeatureCount: statistics.eraseFeatureCount || 0,
  ...
  _createMetadata(metadata) {
    return {
      ...
      algorithm: metadata.algorithm || 'turf-difference',
