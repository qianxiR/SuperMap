...
功能区域组件
    ...
    └── 分析工具面板
        ...
        ├── 叠置分析 (OverlayAnalysisPanel.vue) → useOverlayAnalysis()
...
## 🔗 叠置分析功能 (useOverlayAnalysis)

### **功能描述**
几何叠置分析、操作类型管理、结果处理的组合式函数

### **调用的函数**
- `executeOverlayAnalysis()` - 执行叠置分析
- `performGeometryOverlay(features1, features2, operation)` - 使用 Turf 进行叠加
...
| 叠置分析 | useOverlayAnalysis | AnalysisStore, MapStore | 选中图层、操作类型、分析结果 |
