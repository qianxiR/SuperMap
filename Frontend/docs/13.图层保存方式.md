...
为保证各分析功能在“保存为图层/导出JSON”时有稳定一致的数据来源，前端统一采用“双重数据保存”策略：

- Store 状态保存：用于面板状态与统计展示（例如数量、距离、时间）；
- lastFeatureCollection 本地缓存：用于“保存为图层/导出JSON”的真实数据源。

实现要点：

```ts
// 分析完成后（以某分析为例）
// 1) 保存到 Store（用于面板状态判断/统计显示）
store.setResults(apiResponse) // apiResponse 为标准 FeatureCollection

// 2) 保存到本地 FeatureCollection（用于保存/导出）
lastFeatureCollection.value = apiResponse

// 3) 地图渲染
displayXXXResults(apiResponse)
```

统一的弹窗开启判断（与所有分析面板保持一致）：

```ts
const showLayerNameModal = () => {
  if (!lastFeatureCollection.value ||
      !lastFeatureCollection.value.features ||
      lastFeatureCollection.value.features.length === 0) {
    return
  }
  defaultlayerName.value = generateDefaultName()
...
### 通用：基于 FeatureCollection 的保存（推荐）
当分析后端直接返回标准 FeatureCollection 时，前端应基于 lastFeatureCollection.value 进行统一转换与保存：

```ts
import GeoJSON from 'ol/format/GeoJSON'
import { Feature } from 'ol'

const saveResultsAsLayer = async (layerName: string, sourceType: 'buffer'|'intersect'|'erase'|'path') => {
  ...
  return await saveFeaturesAslayer(olFeatures, layerName, sourceType)
}
```
...
### 擦除分析保存（基于 FeatureCollection）
```ts
// 使用 lastFeatureCollection 进行统一转换
import GeoJSON from 'ol/format/GeoJSON'
import { Feature } from 'ol'

const saveEraseResultsAsLayer = async (layerName?: string) => {
  const fc = lastFeatureCollection.value
  if (!fc || !fc.features || fc.features.length === 0) return false
  ...
  return await saveFeaturesAslayer(olFeatures, layerName || '擦除分析结果', 'erase')
}
```

### 最短路径分析保存（基于 FeatureCollection）
```ts
// 使用 lastFeatureCollection 进行统一转换
const savePathResultsAsLayer = async (layerName?: string) => {
  const fc = lastFeatureCollection.value
  if (!fc || !fc.features || fc.features.length === 0) return false
  ...
  return await saveFeaturesAslayer(olFeatures, layerName || '最短路径分析结果', 'path')
}
```
