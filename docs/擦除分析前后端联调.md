...
for (let i = 0; i < targetFeatures.length; i++) {
  for (let j = 0; j < eraseFeatures.length; j++) {
    ...
    if (difference && difference.geometry) {
      const resultItem = {
        id: `erase_${i}_${j}_${Date.now()}`,
        name: `擦除区域 ${results.length + 1}`,
        geometry: difference.geometry,  // 这是turf.js返回的GeoJSON几何
...
**关键点：**
- `turf.difference()`函数接收FeatureCollection格式的输入
- 返回的`difference`对象本身就是GeoJSON格式的Feature
- 几何数据存储在`difference.geometry`中
...
### 2. 后端擦除计算
- 使用turf.js的`difference`函数进行擦除计算
- 支持异步批处理大量要素组合
- 提供详细的错误处理和日志记录
...
└─────────────────┘    └─────────────────┘    └─────────────────┘
         ...
         │                       │                       ├─────────────────┐
         │                       │                       │                 │
         │                       │                       │ 9. turf.difference(featureCollection)
         │                       │                       │    - 执行几何擦除计算
         │                       │                       │    - 返回擦除后区域GeoJSON
         │                       │                       │    - 处理所有要素组合
         │                       │                       ├─────────────────┐
...
3. **擦除计算**：使用`prepareTurfAnalysisInput()`准备输入，然后调用`turf.difference()`进行几何擦除计算
4. **结果处理**：turf.js返回的本身就是GeoJSON格式，直接用于API响应和前端显示

这个转换过程的关键在于**turf.js本身就是基于GeoJSON标准的**，所以转换过程相对简单，主要是格式验证和几何类型映射。

这个擦除分析功能完全遵循了项目的DDD架构设计，提供了完整的前后端分离实现，支持大量数据的异步处理，并具有良好的性能和用户体验。
